...
if (isset($_POST['createTranslation'])) { 
    ...
    if (!mysqli_stmt_prepare($statement, $sql)) { //If it does not work
        ...
        mysqli_stmt_bind_param($statement, "s", $translation); //Take the input title
        mysqli_stmt_execute($statement);
        mysqli_stmt_store_result($statement); //And store the input
        $checkResult = mysqli_stmt_num_rows($statement); //Get the rows from database

        if($checkResult > 0) { //If the title already exists in the database
            header("Location: create.php?error=translationalreadyexist"); //Display error in the url
            exit();
        } else { //If the title does not exist in the db
            $sql = "INSERT INTO translations (language,createdBy) VALUES (?,?)"; //Passing placeholders
            mysqli_stmt_prepare($statement, $sql);
            mysqli_stmt_bind_param($statement, "si", $translation, $userId);
            mysqli_stmt_execute($statement);
            header("Location: create.php?createtranslation=success");
            exit();   
        }
    }
    mysqli_stmt_close($statement);
